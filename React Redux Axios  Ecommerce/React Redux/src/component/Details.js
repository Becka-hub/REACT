...
const Details = () => {
  ...
  const [produitGet, setProduitGet] = useState(false)
  const history = useHistory()
  useEffect(() => {
    if (id) {
      const getProduit = () => {
        setLoading(true)
        GetProduitDetails(id)
    ...
    return (
      <>
        <div className="col-md-6 my-5">
          <img
            src={'http://127.0.0.1:8000/uploads/' + produit.image}
            width="400px"
          />
        </div>
        <div className="col-md-6 my-5">
          <h1 className="text-uppercase fw-bolder display-5">
            {produit.libelle}
          </h1>
          <hr />
          <p className="lead fw-bolder">
            Rating $ {produit.price}
            <i className="fa fa-start"></i>
          </p>
          <h3 className="fw-bold text-black-50 display-6 my-4">
            {produit.content}
          </h3>
          <button
